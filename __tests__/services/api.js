import { fetchPeople, api } from '~/services/api';
...
describe('Test api service', () => {
  test('should fetch characters', async () => {
    const mockPost = jest.spyOn(api, 'get').mockResolvedValue(people);

    await expect(fetchPeople(1)).resolves.toEqual(people);

    expect(mockPost).toHaveBeenCalledWith('people/?page=1');
  });
});
